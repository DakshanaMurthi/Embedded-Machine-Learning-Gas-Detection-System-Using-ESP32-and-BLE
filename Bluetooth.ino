...
const float thresholdMargin = 150.0;
...
void setup() {
  ...
  // Initialize BLE
  BLEDevice::init("ESP32 Gas Sensor");
...
void loop() {
  ...
  // Send BLE notification if device connected
  if (deviceConnected) {
    String notification = String("Gas: ") + gasNames[lastPrediction] + 
                          " Alarm: " + (alarmActive ? "ON" : "OFF");
